// 🚀 CRT Portal - WebSocket-Enabled Faculty Dashboard
...
export function WebSocketFacultyDashboard() {
  const { user } = useAuth();
  ...
  const setupWebSocketEventHandlers = useCallback(() => {
    ...
    webSocketService.on("faculty_next_session", (data: any) => {
      console.log("⏰ Next session warning:", data);
      if (isComponentMountedRef.current) {
        setCurrentSession((prev) => ({
          ...prev,
          nextSlot: {
  ...
  // Initial data load (only once, then WebSocket takes over)
  const loadInitialData = useCallback(async () => {
    if (!user?.userId || !isComponentMountedRef.current) return;
