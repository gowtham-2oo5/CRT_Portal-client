...
import { ExportAbsenteesExcel } from "./export-absentees-excel";
import { ExcelExportDemo } from "./excel-export-demo";
import { ExportFacultyExcel } from "./export-faculty-excel";
import { ExportPendingFacultyExcel } from "./export-pending-faculty-excel";
...
export function DataExportSettings() {
  ...
  const loadRecentAbsentees = async () => {
    try {
      ...
      const timeSlots = await AttendanceService.getAbsentees(today);
  ...
  const loadPendingFaculty = async () => {
    try {
      ...
      const timeSlots = await AttendanceService.getPendingFaculties(
        pendingDate
      );
      console.log("Actual pending faculty response: ", timeSlots);
      ...
      timeSlots.forEach((slot) => {
        if (!slot.attendancePosted) {
          const facultyId = slot.inchargeFacultyId;
          if (facultyMap.has(facultyId)) {
            const faculty = facultyMap.get(facultyId)!;
            faculty.pendingCount += 1;
          } else {
            // Create faculty entry from slot data
            facultyMap.set(facultyId, {
              id: facultyId,
              name: slot.inchargeFacultyName,
              email: slot.inchargeFacultyEmail,
              phone: slot.inchargeFacultyPhone,
              pendingCount: 1,
            });
          }
        }
            ...
            <TabsContent value="absentees" className="space-y-4 pt-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Export student absentee information including ID, name,
                    email, and phone.
                  </p>
                </div>
                <div className="flex gap-2">
                  ...
                  >
                    ...
                    {isLoadingAbsentees
                      ? "Loading..."
                      : "Load Today's Absentees"}
                  </Button>
                  <ExportAbsenteesExcel
                    absentees={absentees}
