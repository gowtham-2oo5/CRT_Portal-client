...
export class AttendanceValidator {
  ...
  static validateSubmissionRequest(
    ...
    if (request.attendanceRecords) {
      ...
      request.attendanceRecords.forEach((record, index) => {
        ...
        if (studentIds.has(record.studentId)) {
          errors.push(`Duplicate attendance record for student: ${record.name}`);
        }
      ...
      const providedStudentIds = new Set(request.attendanceRecords.map(r => r.studentId));
      
      const missingStudents = students.filter(s => !providedStudentIds.has(s.id));
      const extraStudents = request.attendanceRecords.filter(r => !expectedStudentIds.has(r.studentId));

      if (missingStudents.length > 0) {
        warnings.push(`${missingStudents.length} student(s) not included in attendance: ${missingStudents.map(s => s.name).join(', ')}`);
      }

      if (extraStudents.length > 0) {
        errors.push(`${extraStudents.length} student(s) not enrolled in this section: ${extraStudents.map(r => r.name).join(', ')}`);
      }
    }
    ...
    return {
      ...
      validationRules: {
        ...
        studentValidation: errors.filter(e => e.includes('not enrolled')).length === 0,
   ...
   */
  static validateAttendanceRecord(record: AttendanceRecord, index: number): string[] {
    const errors: string[] = [];
    ...
    if (typeof record.present !== 'boolean') {
      errors.push(`Attendance status invalid for record ${index} (${record.name || 'Unknown'})`);
    }

    // Feedback validation
    if (record.feedback && record.feedback.length > 500) {
      errors.push(`Feedback too long for record ${index} (${record.name || 'Unknown'})`);
    }
  ...
  static canSubmitAttendance(
    ...
    if (unmarked > 0) {
      return { canSubmit: false, reason: `${unmarked} student(s) have unmarked attendance` };
    }
  ...
  static formatValidationErrors(validation: AttendanceValidation): {
    ...
    if (validation.warnings.length > 0) {
      warningMessage = validation.warnings.length === 1
   ...
   */
  static sanitizeSubmissionData(request: SubmitAttendanceRequest): SubmitAttendanceRequest {
    return {
      ...request,
...
export const AttendanceHelpers = {
  ...
  formatAttendanceRecord: (record: AttendanceRecord): string => {
    const status = record.present === true ? 'Present' : record.present === false ? 'Absent' : 'Unmarked';
   ...
   */
  generateExportData: (records: AttendanceRecord[], sessionInfo: {
    sectionName: string;
    date: string;
    timeSlot: string;
    topicTaught: string;
    ...
    return {
      ...
      records: records.map(record => ({
        ...
        name: record.name,
        status: record.present === true ? 'Present' : record.present === false ? 'Absent' : 'Unmarked',
